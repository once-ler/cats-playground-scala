...
class CkEntityInterpreter[F[_]: Async] {
  ...
  def createCompleteEntity[A <: WithEncoder, B <: WithEncoder](inner: A, outer: B)(implicit cf: ClickMockService[F]): F[NodeSeq] = {
   ...
   */
  private def addOrUpdateImpl[A <: CkBase with WithCustomAttributes with WithEncoder with WithFindById, B <: CkBase with WithEncoder](mrn: String = "", fromCk: A, fromCa: A, fromCkCm: B, fromCaCm: B) (implicit cf: ClickMockService[F]): F[NodeSeq] = {
    fromCk.oid match {
      case null | Some("") =>
  ...
  def addOrUpdateNonProject[A <: CkBase with WithCustomAttributes with WithNonProject with WithEncoder with WithFindById, B <: CkBase with WithEncoder](mrn: String = "", fromCk: A, fromCa: A, fromCkCm: B, fromCaCm: B)(implicit cf: ClickMockService[F]): F[NodeSeq] = {
    addOrUpdateImpl(mrn, fromCk, fromCa, fromCkCm, fromCaCm)
  }

  def addOrUpdateProject[A <: CkBase with WithCustomAttributes with WithProject with WithEncoder with WithFindById, B <: CkBase with WithEncoder](mrn: String = "", fromCk: A, fromCa: A, fromCkCm: B, fromCaCm: B)(implicit cf: ClickMockService[F]): F[NodeSeq] = {
    addOrUpdateImpl(mrn, fromCk, fromCa, fromCkCm, fromCaCm)
  }

  def addOrUpdateEntity[A <: CkBase with WithEncoder with WithExplicitTypeName](in: A)(implicit cf: ClickMockService[F]): F[NodeSeq] = {
    in.oid match {
  ...
  def addOrCreateEntityFlow[A <: CkBase with WithEncoder with WithExplicitTypeName](implicit cf: ClickMockService[F]): Pipe[F, A, NodeSeq] = _.evalMap {
    a =>
      a match {
