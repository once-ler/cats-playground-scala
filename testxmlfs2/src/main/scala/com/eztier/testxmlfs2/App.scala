...
class OpenStreetMap[F[_]: ConcurrentEffect: ContextShift] {
  ...
  // For showLinesStdOut
  implicit val showTodo: Show[Place] = Show.show(t => s"${t.Building}\n${t.HouseNumber}\n${t.Road}\n${t.City}\n${t.State}\n${t.Postcode}\n${t.Country}")
  ...
  implicit val placeDecoder: NodeDecoder[Place] = NodeDecoder.decoder(
    xp"./building/text()",
  ...
  def placePipeS[F[_]]: Pipe[F, String, Either[XPathError, List[Place]]] = _.map {
    str =>
      val result: kantan.xpath.XPathResult[List[Place]] = str.evalXPath[List[Place]](xp"//place")
      result
  }
